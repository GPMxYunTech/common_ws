...
int main(int argc, char ** argv)
{
  ...
  std::string map_frame, base_frame,publish_frame;
  ...
  nh_priv.param<std::string>("publish_frame",publish_frame,"/robot_pose_now");
  nh_priv.param<bool>("is_stamped", is_stamped, false);

  if(is_stamped)
    p_pub = nh.advertise<geometry_msgs::PoseStamped>(publish_frame, 1);
  else 
    p_pub = nh.advertise<geometry_msgs::Pose>(publish_frame, 1);
