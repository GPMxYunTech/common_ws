...
groundtruth_2 =  cv2.imread(sys.argv[1],0)
slammap_unaligned_2 =  cv2.imread(sys.argv[2],0)
ret,groundtruth_1=cv2.threshold(groundtruth_2,222,255,cv2.THRESH_BINARY)
ret,slammap_unaligned_1=cv2.threshold(slammap_unaligned_2,222,255,cv2.THRESH_BINARY)
groundtruth = cv2.resize(groundtruth_1, (100, 200), interpolation=cv2.INTER_NEAREST)
slammap_unaligned = cv2.resize(slammap_unaligned_1, (100, 200), interpolation=cv2.INTER_NEAREST)


cv2.imshow('image',groundtruth)
cv2.waitKey(0)
cv2.imshow('image',slammap_unaligned)
cv2.waitKey(0)
