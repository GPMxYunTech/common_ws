...
class Navigation():
    ...
    def done_cb(self,  state, result):
        rospy.loginfo("move_base is done. State: %s, result: %s" % (
    ...
    def move(self, x, y, z, w, _as=None):
        ...
        if b:
            ...
            self.client.send_goal(goal, active_cb=self.active_cb,
                                  feedback_cb=self.feedback_cb, done_cb=self.done_cb)
    ...
    def self_spin(self, z2, w2):
        ...
        yaw_1 = math.atan2(2 * self.rw * self.rz, self.rw *
                           self.rw - self.rz * self.rz)
        if (yaw_1 < 0):
...
class TopologyMapAction():
    ...
    def __init__(self, name):
        ...
        self.Navigation.feedbackEventDef(self.pub_move_base_feedback)
    ...
    def init_param(self):
        global waypoints
        global graph
        self.start_node = rospy.get_param(
            rospy.get_name() + "/start_node", "LD3")
    ...
    def callback_feedback_PBVS(self, feedback):
        if self.show_callback_feedback_count % 50 == 0:
            rospy.loginfo("move_base feedback:%s" % feedback)
        self.show_callback_feedback_count += 1

    def callback_done_PBVS(self,  state, result):
        rospy.loginfo("move_base is done. State: %s, result: %s" % (
            str(state), str(result)))
    ...
    def execute_cb(self, msg):
        ...
        if msg.goal != "" or (msg.target_name != "" and msg.target_pose == None):
            ...
            for i in range(len(path)):
                ...
                if (i > 0 and (waypoints[path[i]][0] == waypoints[path[i-1]][0] and waypoints[path[i]][1] == waypoints[path[i-1]][1])):
                    ...
                    self._feedback.feedback = str(
                        'self_spin from %s to %s' % (path[i-1], path[i]))
                    self._as.publish_feedback(self._feedback)
            ...
            if self.lastpoint == None:
                self.lastpoint = Pose()
            self.lastpoint.position.x = posix
...
class MarkerViewer():
    def __init__(self):
        ...
        self.Marker_Pub = rospy.Publisher(
            "/GoalMarker", Marker, queue_size=100)
        self.TextMarker_Pub = rospy.Publisher(
            "/GoalNameMarker", Marker, queue_size=100)
...
def markpoint():
    ...
    for i in waypoints:
        ...
        rospy.sleep(0.2)
    rospy.loginfo("View Marker Finish")


if __name__ == '__main__':
    rospy.init_node('TopologyMap_server')

    t = Thread(target=markpoint)
    t.start()
    server = TopologyMapAction(rospy.get_name())
    rospy.loginfo("[TopologyMap_server] start")
    rospy.spin()
