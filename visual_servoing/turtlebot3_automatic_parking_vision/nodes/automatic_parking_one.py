...
#############
        self.sub_info_marker = rospy.Subscriber('/ar_pose_marker', AlvarMarkers, self.cbGetMarkerOdom_up, queue_size = 1)
...
##################################################################
    def cbGetMarkerOdom_up(self, markers_odom_msg):
            # posx, posy, thet = self.fnGet2DMarkerPose(marker_odom_msg)
            
            if self.fixed==False:
                if self.set_camera == True:
                    for marker_odom_msg in markers_odom_msg.markers:
                        if marker_odom_msg.id == 2:
                        ...
                        # self.dead_reckoning_marker = pos_x
...
#######################################################################################################################        
    ...
    def fnseqdead_reckoning(self):
        if self.is_triggered == False:
            ...
            self.initial_robot_pose_x = self.robot_2d_pose_x
            self.initial_marker_pose_x = self.marker_2d_pose_x 
        self.dist = abs(self.robot_2d_pose_x - self.initial_robot_pose_x)
        if self.set_camera == True:
            self.dead_reckoning_dist = 0.1
        else:
            self.dead_reckoning_dist = 0.1
            
        self.go_back = abs(self.initial_marker_pose_x)-self.dead_reckoning_dist
        vel = (abs(self.initial_marker_pose_x)-self.dead_reckoning_dist)-self.dist
        # print ("vel = ",vel)
        self.fnGoStraight(vel)
        # print("self.dist = ",self.dist)
        if vel < 0.005:#wanna close num be bigger
            self.fnStop()
            self.is_triggered = False
            return True
        else:
    ...
    def fnSeqfork(self):
        ...
        # else:
        #     return False
        if(self.fork_pose < self.desire_fork-0.005):
            # print("fork up")
            # print("fork_pose ", self.fork_pose)
            # print("desire fork ", self.desire_fork)
            self.fnfork(1)
            return False
        elif(self.fork_pose > self.desire_fork+0.005):
            # print("fork down")
            self.fnfork(-1)
            return False
        else:
    ...
    def fnBack(self):
        ...
        # print(self.go_back)
        if abs(self.robot_2d_pose_x - self.initial_robot_pose_x) > self.go_back:#wanna close num be bigger
            self.fnStop()
            self.is_triggered = False
            return True
        else:
    ...
    def forklayer_2(self):
        self.current_parking_sequence = self.ParkingSequence.setfork.value
        self.desire_fork = 0.40
    def forklayer_2_2(self):
        self.current_parking_sequence = self.ParkingSequence.setfork.value
        self.desire_fork = 0.50
